...
export default class LavaWave extends EventEmitter {
    ...
    static checkOptions(options: LavaWaveOptions) {
        if (typeof options !== 'object') {
            throw new TypeError('LavaWaveOptions must be an object');
        }

        if (!options.nodes) {
            throw new TypeError('LavaWaveOptions must contain a nodes property');
        }

        if (!Array.isArray(options.nodes)) {
            throw new TypeError('LavaWaveOptions.nodes must be an array');
        }

        if (options.nodes.length === 0) {
            throw new TypeError('LavaWaveOptions.nodes must contain at least one node');
        }

        if (!options.sendWS || typeof options.sendWS !== 'function') {
            throw new TypeError('LavaWaveOptions.sendWS must be a function');
        }

        if (options.disabledSources && typeof options.disabledSources !== 'object' && !Array.isArray(options.disabledSources)) {
            throw new TypeError('LavaWaveOptions.disabledSources must be an array');
        }

        if (options.useISRC && typeof options.useISRC !== 'boolean') {
            throw new TypeError('LavaWaveOptions.useISRC must be a boolean');
        }
    }

    /**
     * Create a new LavaWave instance
     * @param {object} options - The LavaWave options
     ...
     */
    constructor(options: LavaWaveOptions) {
        super();

        LavaWave.checkOptions(options);
    ...
    public emit<EventName extends keyof LavaWaveEvents>(event: EventName, ...args: Parameters<LavaWaveEvents[EventName]>): boolean {
        return super.emit(event, ...args);
    }

    public on<EventName extends keyof LavaWaveEvents>(event: EventName, listener: LavaWaveEvents[EventName]): this {
        return super.on(event, listener);
    }

    public once<EventName extends keyof LavaWaveEvents>(event: EventName, listener: LavaWaveEvents[EventName]): this {
