...
export default class Player {
     ...
     */
    constructor(lavawave: LavaWave, options: PlayerOptions) {
        ...
        this.#lavawave.emit('debug', `Player created for guild ${this.guildId}`);
        this.assignNode();

        this.#lavawave.emit('playerCreate', this);
    }
    ...
    private async assignNode() {
        const node = await this.#lavawave.bestNode();

        this.node = node;
        this.#lavawave.emit('debug', `Assigned node ${node.identifier} to player ${this.guildId}`);
    }
    ...
    public addTracks(tracks: Track | UnresolvedTrack | Array<Track | UnresolvedTrack>, requester: User) {
        ...
        this.#lavawave.emit('trackAdd', this, tracks);
    }
    ...
    public async connect() {
        ...
        this.#lavawave.emit('debug', `Connecting player ${this.guildId} to voice channel ${this.voiceChannelId}`);
        ...
        this.#lavawave.emit('playerConnect', this);
    }
     ...
     */
    public disconnect() {
        this.#lavawave.sendWS(this.guildId, {
            op: 4,
            d: {
        ...
        this.#lavawave.emit('debug', `Player ${this.guildId} disconnected from voice channel`);
    }
    ...
    public destroy() {
        ...
        this.node?.rest.destroyPlayer(this.guildId)
            .catch((_error) => {
                this.#lavawave.emit('error', this.node!, new Error(`Failed to send destroyPlayer signal to node "${this.node?.identifier}"`));
            });

        this.node = null;
        this.#lavawave.players.delete(this.guildId);

        this.#lavawave.emit('playerDestroy', this);
        this.#lavawave.emit('debug', `Player ${this.guildId} destroyed`);
    }
    ...
    public async moveNode(node: Node) {
        ...
        this.#lavawave.emit('debug', `Moving player ${this.guildId} from node ${this.node?.identifier ?? 'None'} to node ${node.identifier}`);
            ...
            if (newTrack) {
                if (newTrack instanceof UnresolvedTrack) {
                    try {
                        ...
                        this.#lavawave.emit('trackException', this, newTrack, (err as LoadException & { cause: string }));
    ...
    private sendVoiceState() {
        ...
        this.#lavawave.emit('debug', `Sent voiceStateUpdate to discord gateway for player ${this.guildId}. Channel: ${this.voiceChannelId}. Self mute: ${this.selfMute}. Self deaf: ${this.selfDeaf}`);
    }
    ...
    public async sendVoiceUpdate() {
        ...
        this.#lavawave.emit('debug', `Sent voiceUpdate to lavalink node for player ${this.guildId}.`);
    }
