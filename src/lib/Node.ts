...
export default class Node {
     ...
     * @param {LavaWave} lavawave - The LavaWave instance
     ...
     */
    constructor(lavawave: LavaWave, options: NodeOptions) {
    ...
    public connect() {
        ...
        const headers = {
            'User-Id': this.#lavawave.clientId,
            'Client-Name': `LavaWave/${VERSION}_${generateRandomKey(8)}`,
     ...
     */
    public disconnect() {
        if (this.#ws !== null) this.#ws.close(1000, 'LavaWave: disconnect');
    }
    ...
    public async updatseNodeSession(): Promise<boolean> {
        try {
            ...
            this.#lavawave.emit('error', this, new Error(`Updating session failed, on node "${this.options.id}"`));
            return false;
        }
    }
    ...
    private KeepingNodeAwake(milliseconds: number) {
        this.#keepAliveInterval = setInterval(async () => {
            try {
                ...
                this.#lavawave.emit('error', this, new Error(`Keeping node awake failed, try to reconnect node "${this.options.id}"`));
    ...
    public async getPing(timeout: number = 1500): Promise<number> {
        try {
            ...
            this.disconnect();
            this.#lavawave.emit('error', this, new Error(`An error occurred while updating stats: Unable to connect to the node`));
            return -1;
        }
    }
    ...
    public async updateStats(timeout: number = 1500): Promise<void> {
        try {
            ...
            this.disconnect();
            this.#lavawave.emit('error', this, new Error(`An error occurred while updating stats: Unable to connect to the node "${this.identifier}"`));
        }
    }
    ...
    private async pollTrack(player: Player) {
        ...
        if (newTrack) {
            if (newTrack instanceof UnresolvedTrack) {
                try {
                    ...
                    this.#lavawave.emit('trackException', player, newTrack, (err as LoadException & { cause: string }));
        ...
        player.current = null;
        this.#lavawave.emit('queueEnd', player);
    }

    private handlePlayerEvent(e: PlayerEventPayload) {
        const player = this.#lavawave.players.get(e.guildId);
        ...
        switch (e.type) {
            ...
            default: {
                this.#lavawave.emit('warn', this, `Unhandled player event. Unknown event type: ${e.type}`);
                break;
            }
        }
    }
    ...
    private handleTrackStart(_: TrackStartEvent, player: Player) {
        ...
        this.#lavawave.emit('trackStart', player, player.current!);
    }
    ...
    private handleTrackEnd(ev: TrackEndEvent, player: Player) {
        ...
        if (['LOAD_FAILED', 'CLEANUP'].includes(ev.reason)) {
            this.#lavawave.emit('trackEnd', player, player.current!, ev.reason);
    ...
    private handleTrackStuck(ev: TrackStuckEvent, player: Player) {
        this.#lavawave.emit('trackStuck', player, player.current!, ev.thresholdMs);
    }

    private handleTrackException(ev: TrackExceptionEvent, player: Player) {
        this.#lavawave.emit('trackException', player, player.current!, ev.exception);
        player.skip();
    }

    private handleWSClose(ev: WebSocketClosedEvent, player: Player) {
        this.#lavawave.emit('playerDisconnect', player, ev.code, ev.reason);
    ...
    private open() {
        ...
        this.#lavawave.emit('nodeConnect', this);
    ...
    private message({ data }: MessageEvent) {
        ...
        switch (payload.op) {
            ...
            case 'stats': {
                ...
                this.calcPenalties();
                this.#lavawave.emit('debug', `Node "${this.identifier}" penalties: ${this.totalPenalties}`);
                break;
            }
            case 'playerUpdate': {
                this.#lavawave.players.get(payload.guildId)?.update(payload.state);
                break;
            }
            case 'event': {
            ...
            default: {
                this.#lavawave.emit('warn', this, 'Unknown payload op: ' + payload.op);
                break;
            }
        }

        this.#lavawave.emit('raw', this, payload);
    }
    ...
    private error({ error, message }: ErrorEvent) {
        ...
        if (message.includes('401')) {
            ...
            this.#lavawave.emit('error', this, new Error('Authentication failed!'));
            return;
        }
        this.#lavawave.emit('error', this, error);
    }
    ...
    private async close({ code, reason, wasClean }: CloseEvent) {
        ...
        if (wasClean) {
            this.#lavawave.emit('nodeDisconnect', this, code, reason);
            return;
        }

        try {
            const newNode = await this.#lavawave.bestNode();

            if (newNode) {
                for (const player of this.#lavawave.players.values()) {
                    if (player.node === this) {
        ...
        this.#lavawave.emit('error', this, new Error(`WebSocket closed abnormally with code ${code}.`));
    ...
    private upgrade(msg: IncomingMessage) {
        if (msg.headers['session-resumed'] === 'true') {
            this.#lavawave.emit('nodeResume', this);
        }
    }

    private pong(data: Buffer) {
        this.#lavawave.emit('pong', this, data);
    }
}
